...
│   └── index.html            # 招聘复盘看板：漏斗、来源、流失原因与时效数据
└── skills/                   # 可移植的 AI Skill 定义
```

同一岗位的候选人流程信息（例如所处阶段、约面日期、终止原因）统一以 Excel 台账为准；每位候选人的简历证据、电话纪要和面试反馈另存为 Markdown 档案。`index.html` 是从台账生成的**招聘复盘看板**：用于查看招聘漏斗、阶段人数与时效、简历来源、终止/流失原因和待跟进事项。请先更新 Excel，再刷新看板，不要在看板中直接修改流程信息。
...
4. 用台账生成该岗位的招聘复盘看板 `index.html`，查看招聘漏斗、阶段时效、简历来源、流失原因和待办。
