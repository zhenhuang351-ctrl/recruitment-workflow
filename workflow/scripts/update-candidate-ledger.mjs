...
export function mergeCandidate(candidate) { const patch = { ...decisionToLedgerPatch(candidate), ...candidate.fields }; for (const key of ["简历收取时间", "下次跟进日期", "Offer发出日期", "Offer接受日期", "预计入职日期", "实际入职日期", "一面日期", "二面日期", "三面日期", "HRBP日期", "决策会日期"]) if (typeof patch[key] === "string" && /^\d{4}-\d{2}-\d{2}$/.test(patch[key])) patch[key] = new Date(`${patch[key]}T00:00:00`); return patch; }
